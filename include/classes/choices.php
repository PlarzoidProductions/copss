...
class Choices {
	...
	function getGameTypeChoices(){
		//1 if it gets considered for all the achievements, 0 if it doesn't
		//top most item will be the default option, so should be WM/H
		return array(
			array("text"=>"Warmachine/Hordes", "value"=>1),
			array("text"=>"High Command", "value"=>0),
			array("text"=>"Bodgers Game", "value"=>0),
			array("text"=>"IKRPG Demo", "value"=>0),
			);
		}
