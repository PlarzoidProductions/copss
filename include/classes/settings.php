...
class Settings
{
	...
	*to the object.
	...
	public function makeSettings($settings){
		$setting_fields=array('name', 'teamgame', 'newopponent', 'outofstate', 'fullypainted', 'fullypaintedall', 'scenariotable',
			'played25', 'played35', 'played50', 'played75', 'played100', 'playedUNBOUND', 'playedall',
...
//var_dump($query);
	...
	function getPointsForNumPlayers($n){
		$query = "Select numplayers";

		$code = $this->queryDB($query);

		var_dump($code);
	}

	function getPointsForNumGames($n){
		$query = "Select numgames";
	...
	function getPointsForNumFactions($n){

	}

	function getPointsForNumLocations($n){

	}
		
	private function queryDB($query) {
		$settings_db=mysql_connect($this->server, $this->mysql_user, $this->mysql_user_pass) or die(mysql_error());
